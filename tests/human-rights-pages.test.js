...
const dashboardScript = fs.readFileSync(
  path.join(root, "public", "human-rights-dashboard.js"),
  "utf8",
);
...
test("server exposes human-rights APIs and CSP-enabled security headers", () => {
  assert.match(
    serverOffline,
    /contentSecurityPolicy:\s*CONTENT_SECURITY_POLICY/i,
  );
  ...
  assert.match(
    serverOffline,
    /versionedConsent:\s*getConsentDashboard\(sessionId\)/i,
  );
...
test("chat UI links to the new human-rights dashboards", () => {
  ...
  assert.match(indexHtml, /href="\/data-dashboard"/i);
...
test("all requested human-rights pages exist and share the dashboard runtime", () => {
  for (const page of expectedPages) {
    ...
    assert.match(
      html,
      /human-rights-dashboard\.js/i,
      `${page} should load the shared dashboard runtime`,
    );
    assert.match(
      html,
      /id="dashboard-root"/i,
      `${page} should render the shared dashboard root`,
    );
  }
